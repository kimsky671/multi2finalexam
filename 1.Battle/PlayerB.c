...
void custom_strcat(char* dest, const char* src, size_t dest_size) {
    ...
    while (*src != '\0' && (d - dest) < (int)dest_size - 1) {
        *d++ = *src++;
    }
    *d = '\0';
}
...
// [ 전투 ]
//  거리를 계산해 가며 하는 공격형
// * 상대 거리(1 vs 2~4)에 따라 최적의 공격 스킬을 사용하며 무조건 추격.
// =================================================================================================
...
int student2_ai_logic(const Player* my_info, const Player* opponent_info) {
    ...
    if (dist <= 1) {
        ...
        if (poison_used == 0 && my_mp >= 5 && is_skill_unlocked(my_temp_key, CMD_POISON)) {
            poison_used = 1;
            return CMD_POISON;
        }

        
        if (my_mp >= 2 && is_skill_unlocked(my_temp_key, CMD_STRIKE)) {
            return CMD_STRIKE;
        }
    ...
    if (dist >= 2 && dist <= 4) {
        ...
        if (my_mp >= 1 && is_skill_unlocked(my_temp_key, CMD_RANGE_ATTACK)) {
...
void student2_ai_entry() {
    ...
    (void)register_player_ai("TEAM-BETA", student2_ai_logic);
    printf("TEAM-BETA [System]: AI 로직 등록 완료.\n"); 

    load_quiz_1_answer();
    load_quiz_2_answer();

    printf("TEAM-BETA: 모든 준비 완료. 계속하려면 아무 키나 누르십시오.\n");
    (void)getchar();
}